angular.module("MyApp", []).controller("MyController", function($scope,$http) {
    ...
    $scope.coin_details = function()
    {
        var success_cb = function(data)
        {
            $scope.released_coins = data.message.coin_details.number_of_released_coins;
            if(!$scope.$$phase) $scope.$apply();
        }
        var request_data = {};
        SSK.site_call("AJAX",window._site_url+"coins/coin_details",request_data,success_cb);
    }
    ...
    $scope.wallet_transfer = function()
    {
        ...
        if(confirm("Do you want to transfer "+coins+" coins ?"))
        {
            ...
            SSK.site_call("AJAX",window._site_url+"coins/wallet_transfer_request",request_data,success_cb);    
        }   
    }
    ...
    $scope.wallet_transfer_otp = function()
    {
        ...
        request_data['transfer_username'] = transfer_username;
        SSK.site_call("AJAX",window._site_url+"coins/wallet_transfer_otp",request_data,success_cb);    
