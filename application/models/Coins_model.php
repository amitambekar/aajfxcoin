...
class Coins_model extends CI_Model
{
    ...
    function wallet_transfer_coins($transfer_userid,$userid,$coins,$amount,$coin_price,$payment_details,$created_date){
        $this->db->trans_start();
        $array = array('userid' => $userid,'from_user'=>$transfer_userid,'amount'=>$amount,'coin_price'=>$coin_price,'coins'=>$coins,'description'=>'Wallet Transfer Debit','purchase_date'=> $created_date);
        $this->db->set($array);
        $this->db->insert('user_coins');
        ...
        $this->db->trans_start();
        $array = array('userid' => $transfer_userid,'from_user'=>$userid,'amount'=>$amount,'coin_price'=>$coin_price,'coins'=>$coins,'payment_details'=>$payment_details,'description'=>'Wallet Transfer Credit','purchase_date'=> $created_date);
        $this->db->set($array);
        $this->db->insert('user_coins');
